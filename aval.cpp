...
void save(vector<Heyvan> heyvan)
{
    ...
    cin >> namefile;
    ofstream fileacho;
    fileacho.open(namefile, ios::out | ios::trunc);
    for(int i = 0; i < heyvan.size(); i++) {
        fileacho << heyvan.at(i).name << ' ' << heyvan.at(i).cage_number << ' ' << heyvan.at(i).year_of_entry << ' ' << heyvan.at(i).is_rare << ' ' << heyvan.at(i).is_fed << endl;
    }
    fileacho.close();
...
void load(vector<Heyvan> &heyvan){
    ...
    cin >> namefile;
    ifstream fileacho;
    fileacho.open(namefile, ios::in);
    if(fileacho) {
        Heyvan h;
    while (fileacho >> h.name >> h.cage_number >> h.year_of_entry >> h.is_rare >> h.is_fed){
        heyvan.push_back(h);
    }
    fileacho.close();
